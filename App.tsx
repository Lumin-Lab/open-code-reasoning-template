...
const App: React.FC = () => {
  ...
  const handleManualFetch = async () => {
    ...
    // If this is the first message, include the coding question as context
    const history = messages.length === 0
      ? [
        {
          role: 'user',
          parts: [{
            text: `Here is the code snippet to discuss:\n\n\`\`\`javascript\n${activeTopic.code}\n\`\`\`\n\nContext:\n- Title: ${activeTopic.title}\n- Description: ${activeTopic.description}\n- Pre-conditions: ${activeTopic.preConditions}\n- Post-conditions: ${activeTopic.postConditions}\n- Invariants: ${activeTopic.invariants}\n\nLet's start the debate about this code.`
          }]
        }
      ]
      : messages.map(m => ({
        role: m.speaker === Speaker.Tutor ? 'model' : 'user',
        parts: [{ text: m.text }]
      }));

    // Determine last speaker to prompt for the other
    const lastMsg = messages[messages.length - 1];
    const lastSpeaker = lastMsg ? lastMsg.speaker : Speaker.Student;
    const effectiveLastSpeaker = lastSpeaker;

    // Determine who will speak next
    const nextSpeaker = effectiveLastSpeaker === Speaker.Tutor ? Speaker.Student : Speaker.Tutor;
    ...
    try {
      ...
      const response = await fetch(apiEndpoint, {
        ...
        body: JSON.stringify({
          history,
          lastSpeaker: effectiveLastSpeaker
        })
      });
      ...
      alert(`Failed to fetch response from ${nextSpeaker === Speaker.Student ? 'Hugging Face' : 'Gemini'}.`);
    } finally {
          ...
          <div className="p-6 flex-1 overflow-y-auto space-y-6">
            ...
            <div>
              ...
              <div className="space-y-6">
                ...
                >
                  {isGenerating ? (
                    <span className="w-5 h-5 border-2 border-[#50E3C2] border-t-transparent rounded-full animate-spin"></span>
                  ) : (
                    <span className="material-symbols-outlined">psychology</span>
                  )}
                  {isGenerating ? 'Thinking...' : 'ask Tutor to Debug'}
                </button>

                <button
                  onClick={togglePlay}
                  className={`w-full flex items-center justify-center gap-2 min-w-[84px] cursor-poiner rounded-lg h-12 px-4 ${isPlaying ? 'bg-red-500/20 text-red-400 border border-red-500/50' : 'bg-[#9F70FD] text-white hover:bg-[#9F70FD]/90'} text-base font-bold transition-all`}
                >
              ...
              return (
                <div key={msg.id} className={`flex items-start gap-4 max-w-xl message-enter ${isTutor ? '' : 'ml-auto justify-end'}`}>
                  ...
                  <div className={`flex flex-col gap-1 ${isTutor ? 'items-start' : 'items-end'}`}>
                    ...
                    <div className={`text-base font-normal leading-relaxed rounded-2xl px-5 py-3 shadow-md ${isTutor
            ...
            {isThinking && (
              <div className={`flex items-start gap-4 max-w-xl ${currentSpeaker === Speaker.Student ? 'ml-auto justify-end' : ''}`}>
                <div className={`flex flex-col gap-1 ${currentSpeaker === Speaker.Student ? 'items-end' : 'items-start'}`}>
                  ...
                  <div className={`text-base font-normal leading-normal rounded-2xl px-4 py-3 ${currentSpeaker === Speaker.Tutor
